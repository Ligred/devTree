...
export const TreeView = React.forwardRef<HTMLDivElement, TreeProps>(
  (
    ...
    return (
      <div
        className={cn('relative overflow-hidden p-2', draggedItem && 'is-dragging', className)}
      >
        <TreeItem
          data={data}
          ref={ref}
        ...
        <div
          ...
          className="mt-1 min-h-10 w-full rounded-md border-2 border-dashed border-transparent bg-transparent px-2 py-2 text-center text-xs text-muted-foreground transition-colors in-[.is-dragging]:border-primary/50 in-[.is-dragging]:bg-primary/10 in-[.is-dragging]:text-foreground"
          onDragOver={(e) => {
            e.preventDefault();
            e.stopPropagation();
      ...
      <AccordionPrimitive.Item value={item.id}>
        <AccordionPrimitive.Header>
          <AccordionPrimitive.Trigger
            ...
            onDrop={(e) => { e.preventDefault(); setIsDragOver(false); handleDrop?.(item); }}
          >
            {/**
            ...
            <ChevronRight
              className={cn(
                'mr-1 h-4 w-4 shrink-0 text-muted-foreground/60 transition-transform duration-200',
                isOpen && 'rotate-90'
              )}
            />
            {renderItem ? (
              ...
              <>
                <TreeIcon item={item} isOpen={isOpen} isSelected={isSelected} default={defaultNodeIcon} />
                ...
                {item.actions && (
                  ...
                  <span className="ml-auto flex shrink-0 opacity-0 transition-opacity group-hover/row:opacity-100">{item.actions}</span>
                )}
              </>
            )}
          </AccordionPrimitive.Trigger>
        ...
        <AccordionPrimitive.Content className="overflow-hidden text-sm data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down">
           ...
           */}
          <div className="ml-3 border-l border-border/40 pl-1 pb-1">
            <TreeItem
            ...
            {/* Empty folder hint */}
            {!hasChildren && (
              <p className="py-1 pl-2 text-xs text-muted-foreground/50 italic">
                Empty folder
              </p>
            )}
          </div>
...
>(
  (
    ...
    const onDragStart = (e: React.DragEvent) => {
      if (!item.draggable || item.disabled) { e.preventDefault(); return; }
      e.dataTransfer.setData('text/plain', item.id);
      handleDragStart?.(item);
    };
    ...
    return (
      <div
        ...
        onClick={() => { if (!item.disabled) { handleSelectChange(item); item.onClick?.(); } }}
