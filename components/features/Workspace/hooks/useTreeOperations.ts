'use client';
/* eslint-disable sonarjs/void-use -- intentional fire-and-forget optimistic API calls */
/**
    ...
    (parentId: string) => {
      ...
      void apiCreatePage(fileTitle, dbFolderId)
        .then((created) => {
          ...
          const replaceLocalId = (p: Page) => (p.id === localPageId ? { ...p, id: created.id } : p);
          setPages((prev) => prev.map(replaceLocalId));
  ...
  const handleDocumentDrag = useCallback(
    (sourceItem: TreeDataItem, targetItem: TreeDataItem) => {
      setTreeRoot((root) => {
        ...
        const resolvedFolderId = isTargetRoot || targetId === ROOT_DROP_TARGET_ID ? null : targetId;
        ...
        if (isPage) {
          void apiMovePage(sourceItem.id, { folderId: resolvedFolderId, order }).catch((err) =>
            console.error('[movePage]', err),
          );
        } else {
          void apiMoveFolder(sourceItem.id, { parentId: resolvedFolderId, order }).catch((err) =>
            console.error('[moveFolder]', err),
          );
        }
