...
export function useSaveLogic({
  ...
  const handleSave = useCallback(async () => {
    ...
    if (idMap.size > 0) {
      setPages((prev) =>
        prev.map((p) => (p.id === activePageId ? { ...p, blocks: reconciledBlocks } : p)),
      );
    }
  ...
  const handleLeaveWithout = useCallback(() => {
    ...
    if (pendingNavId) {
      ...
      if (pageSnap && activePageId) {
        ...
        setPages((prev) => prev.map((p) => (p.id === activePageId ? { ...p, blocks: snap } : p)));
      }
      setIsDirty(false);
      setActivePageId(pendingNavId);
      setMobileSidebarOpen(false);
  ...
  const handleTitleBlur = useCallback(() => {
    ...
    titleBlurTimerRef.current = setTimeout(() => {
      titleBlurTimerRef.current = null;
      // eslint-disable-next-line sonarjs/void-use -- fire-and-forget with .catch() error handling below
  ...
  const handleTagsChange = useCallback(
    (tags: string[]) => {
      if (!activePageId) return;
      setPages((prev) => prev.map((p) => (p.id === activePageId ? { ...p, tags } : p)));
  ...
  const handleBlocksChange = useCallback(
    (blocks: Block[]) => {
      if (!activePageId) return;
      setPages((pages) => pages.map((p) => (p.id === activePageId ? { ...p, blocks } : p)));
      setIsDirty(true);
    },
