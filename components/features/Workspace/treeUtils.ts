 ...
 */
export function buildTreeRootFromApi(folders: ApiFolderStub[], pages: ApiPageStub[]): TreeRoot {
 ...
 */
export function renameNode(root: TreeRoot, nodeId: string, name: string): TreeRoot {
 ...
 */
export function insertNodeUnder(root: TreeRoot, parentId: string, node: TreeNode): TreeRoot {
...
export function normalizeDropTargetId(targetId: string, rootId: string): string {
  if (targetId === '' || targetId === 'parent_div' || targetId === ROOT_DROP_TARGET_ID) {
    return rootId;
  }
  return targetId;
}
...
function wouldCreateCycle(root: TreeRoot, sourceNodeId: string, targetId: string): boolean {
 ...
 */
export function moveNode(root: TreeRoot, sourceNodeId: string, targetParentId: string): TreeRoot {
  ...
  function replaceInChildren(children: TreeNode[]): TreeNode[] {
    return children.map((node) => {
      if (node.id === oldId) {
        ...
        return isLeaf ? { ...node, id: newId, pageId: newId } : { ...node, id: newId };
      }
