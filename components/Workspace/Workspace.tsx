...
export function Workspace({ initialRoutePageId }: WorkspaceProps) {
  ...
  const activeFolderIdForTracking = useMemo(
    () =>
      activePageId
        ? (pages.find((p) => p.id === activePageId)?.folderId ?? undefined)
        : undefined,
  ...
  const handleBreadcrumbClick = useCallback(
    (nodeId: string) => {
      ...
      if (!node) return;
      if (node.pageId) { handleSelect(node.pageId); return; }
  ...
  const renderTreeItem = useCallback(
    (params: { item: TreeDataItem; isLeaf: boolean; isSelected: boolean; hasChildren: boolean }) => (
      <FolderRenameRow
        item={params.item}
  ...
  return (
    <div className="flex h-full overflow-hidden bg-background font-sans text-foreground">
      ...
      {leftPanelHidden ? (
        ...
        >
          ...
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <h1 className="text-xl font-semibold tracking-tight text-primary">{t('app.title')}</h1>
            <button
              type="button"
      ...
      <DeleteConfirmDialog
        ...
        onOpenChange={(open) => { if (!open) treeOps.setDeleteDialog(null); }}
